...
const activeMajorTypeId = ref<string | null>(null);
...
function selectMajorType(): MonsterMajorTypeData {
  return monsterRollTableData.majorTypes.find((majorType) => majorType.id === activeMajorTypeId.value) ??
    selectRandomItem(monsterRollTableData.majorTypes);
}

function createMonster(): GeneratedMonster {
  const majorType = selectMajorType();
...
function onSelectMajorType(majorTypeId: string) {
  activeMajorTypeId.value = majorTypeId;
  generatedMonster.value = null;
}
</script>
    ...
    <UiPanel
      ...
      <div class="mt-3 border-t border-hairline pt-3">
        <div class="flex flex-wrap gap-2">
          <UiButton
            v-for="majorType in monsterRollTableData.majorTypes"
            :key="majorType.id"
            :data-major-type="majorType.id"
            size="sm"
            :variant="activeMajorTypeId === majorType.id ? 'outline' : 'secondary'"
            @click="onSelectMajorType(majorType.id)"
          >
            {{ majorType.label }}
          </UiButton>
        </div>
        <div class="mt-3 flex items-center justify-between gap-2 rounded-2 border border-hairline p-2">
          ...
          <div class="flex gap-2">
            <UiButton
              ...
              variant="primary"
              @click="onGenerateMonster">
              생성하기
            </UiButton>
            <UiButton
              data-action="reset"
              variant="secondary"
              @click="onResetMonster">
              초기화
            </UiButton>
          </div>
        </div>
      ...
      <div
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 border-b border-hairline pb-3">
          ...
          <div class="flex gap-2">
            <UiButton
              size="sm"
              @click="onGenerateMonsterCore">
              핵심 재생성
            </UiButton>
            <UiButton
              size="sm"
              @click="onGenerateEncounterStructure">
              조우 재생성
            </UiButton>
          </div>
        </div>
        ...
        <div class="mt-3">
          ...
          <div class="mt-1 flex flex-wrap gap-1">
            <span
              v-for="behavior in generatedMonster.behaviors"
              :key="behavior.id"
        ...
        <div class="mt-3">
          ...
          <div class="mt-1 flex flex-wrap gap-1">
            <span
              v-for="combatStyle in generatedMonster.combatStyles"
              :key="combatStyle.id"
