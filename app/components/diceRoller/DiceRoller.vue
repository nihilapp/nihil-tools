...
<template>
  <section
    ...
    <form
      ...
      class="flex flex-wrap items-end gap-2 rounded-2 border border-hairline bg-surface p-3"
      ...
      <label class="flex min-w-20 flex-1 flex-col gap-1 text-sm font-600 text-ink">개수
        <input
          ...
          min="1"
          type="number"
          class="rounded-2 border border-hairline bg-surface px-3 py-2 text-sm outline-none focus:border-primary"
    ...
    <div class="min-h-0 flex-1 overflow-y-auto rounded-2 border border-hairline bg-canvas-soft p-2">
      <div
        v-if="activeHistory.length > 0"
        class="flex flex-col gap-2"><DiceRollerResult
