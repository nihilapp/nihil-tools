...
import { buildAppNavigation, findAppById, resolvePageMeta } from '../../utils/app-shell';
...
const activeApp = computed(() => findAppById(String(route.params.id ?? ''), appItems));
const pageMeta = computed(() => resolvePageMeta(route.path, appItems.length, activeApp.value?.name));

const cssVariants = cva(
  [
    'sticky top-0 z-10 border-b border-black-200 bg-black-100/90 backdrop-blur-sm',
  ],
  {
    variants: {},
    compoundVariants: [
